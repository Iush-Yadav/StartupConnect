...
export default function PostCard({ post, onLike }: PostCardProps) {
  // --- DEBUG LOGS START ---
  console.log(`PostCard (ID: ${post.id}): Render - user_has_followed = ${post.user_has_followed}, likes = ${post.likes}`);
  console.log(`PostCard (ID: ${post.id}): Post object on render:`, post);
  // --- DEBUG LOGS END ---
            ...
            <AnimatePresence>
              {showShareMenu && (
                <motion.div
                  ...
                  transition={{ duration: 0.2 }}
                  ref={shareMenuRef}
                  className="absolute bottom-full mb-2 right-0 bg-white border border-gray-200 rounded-lg shadow-lg py-2 px-3 z-10 flex flex-col items-start min-w-[150px]"
                >
                  <h4 className="text-xs text-gray-500 mb-2">Share Post:</h4>
                  <div className="flex flex-wrap gap-2">
                    {/* WhatsApp */}
                    <div className="relative flex items-center">
                      <button
                        onClick={() => handleShare('whatsapp')}
                        className="relative group flex items-center p-1 rounded-lg hover:bg-green-600/10 transition-colors justify-start"
                      >
                        <div className="w-7 h-7 rounded-full bg-green-50 flex items-center justify-center transition-colors flex-shrink-0">
                          <Send className="h-4 w-4 text-green-700" />
                        </div>
                        <span className="ml-2 w-0 overflow-hidden opacity-0 group-hover:w-max group-hover:opacity-100 group-hover:overflow-visible transition-all duration-200 whitespace-nowrap pointer-events-none text-sm font-medium text-gray-700 flex-shrink-0">WhatsApp</span>
                      </button>
                    </div>
