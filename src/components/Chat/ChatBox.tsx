    ...
    const setupChat = async () => {
      ...
      const subscription = supabase
        ...
        .on('postgres_changes', {
          ...
          console.log('ChatBox: Real-time payload received:', payload);
          if (payload.eventType === 'INSERT') {
            const newMessage = payload.new as Message;
            console.log('ChatBox: New message INSERT received:', newMessage);
            setMessages(prev => {
              if (!prev.some(msg => msg.id === newMessage.id)) {
                console.log('ChatBox: Adding new message to state:', newMessage);
                return [...prev, newMessage];
              } else {
                console.log('ChatBox: Message already in state, skipping:', newMessage);
              }
              return prev;
            });
            ...
            if (newMessage.receiver_id === currentUser.id && !newMessage.is_read) {
              console.log('ChatBox: Marking message as read and fetching total unread messages.');
              supabase
                .from('messages')
                .update({ is_read: true })
                .eq('id', newMessage.id)
            ...
            console.log('ChatBox: Message UPDATE received:', payload.new);
            ...
            console.log('ChatBox: Message DELETE received:', payload.old);
